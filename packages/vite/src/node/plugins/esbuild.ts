...
// 下面代码先是整合 ESbuild 的配置项，然后通过 ESbuild 的transform 方法将 ts 文件编译成 js 文件，最后返回编译后代码以及 sourcemap 相关信息。
  ...
  return {
    ...
    async transform(code, id) {
      if (filter(id) || filter(cleanUrl(id))) {
        // 匹配 ts、tsx、jsx 文件 当匹配成功之后，调用transformWithEsbuild转换插件，这其实也是Vite快的一个原因之一，通过 ESbuild 转换代码。
