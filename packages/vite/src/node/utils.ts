...
/**
在函数内部，首先使用 path.posix.normalize 方法对输入的路径进行规范化处理。这个方法会将路径中的斜杠（\）转换为正斜杠（ /），并删除路径中的冗余部分，如连续的正斜杠或根目录。
然后，根据当前操作系统是否为 Windows，使用 slash 函数对路径进行预处理。在 Windows 操作系统中，路径分隔符通常是反斜杠（\），而在其他操作系统中通常是正斜杠（ /）。slash 函数将反斜杠转换为正斜杠，以便在后续的处理中使用统一的路径分隔符。
最后，返回规范化后的路径。
通过使用这个函数，可以确保输入的路径在不同的操作系统和环境中都能被正确解析和处理，避免了由于路径分隔符不一致而导致的错误。
 */
...
// 函数首先检查 path 的最后一个字符是否为斜杠。如果不是，则在字符串的末尾添加一个斜杠，然后返回结果。
// 如果 path 的最后一个字符已经是斜杠，则函数直接返回 path，不做任何修改。
...
// 例如，如果我们调用 cleanUrl('http://example.com/?query#fragment')，函数将返回 "http://example.com/"。
const postfixRE = /[?#].*$/s
...
export const isJSRequest = (url: string): boolean => {
  ...
  // 如果 url 没有扩展名，并且最后一个字符不是斜杠，函数也返回 true，因为这种情况下也可能是一个 JavaScript 请求。
