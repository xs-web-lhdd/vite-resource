 ...
 */
// vite CLI中 vite optimize 命令就是调用的这个函数
export async function optimizeDeps(
  config: ResolvedConfig,
  force = config.optimizeDeps.force, // 设置为 true 强制使依赖预构建
  asCommand = false, // 在命令行被调用？
 ...
 */
// 创建初始的依赖项优化元数据，如果存在依赖项缓存并且没有强制进行预捆绑，则从缓存中加载它。
export async function loadCachedDepOptimizationMetadata(
  ...
  if (firstLoadCachedDepOptimizationMetadata) {
    ...
    // 如果较早的进程提前退出，请启动对过时的处理依赖项目录的清理。
    setTimeout(() => cleanupDepsCacheStaleDirs(config), 0)
  }

  // 'C:/Users/LiuHao/Desktop/temp/test-vite/node_modules/.vite/deps'
  const depsCacheDir = getDepsCacheDir(config, ssr)

  // 没有强制刷新依赖预构建，就从缓存中取出_metadata.json里面的内容然后返回
  if (!force) {
    let cachedMetadata: DepOptimizationMetadata | undefined
    try {
      // 拿到 _metadata.json 的路径
      const cachedMetadataPath = path.join(depsCacheDir, '_metadata.json')
      // 拿到 _metadata.json 里面的内容
      cachedMetadata = parseDepsOptimizerMetadata(
        await fsp.readFile(cachedMetadataPath, 'utf-8'),
        depsCacheDir,
      )
    } catch (e) { }
    // 通过hash判断内容有没有更新，内容如果不变，就直接返回之前的缓存数据 cachedMetadata 即可
  ...
  // 删除指定的依赖项缓存目录（depsCacheDir）及其子目录中的所有文件。
...
export function runOptimizeDeps(
  ...
  // 依赖的文件目录 'C:/xx/xx/node_modules/.vite/deps'
  const depsCacheDir = getDepsCacheDir(resolvedConfig, ssr)
  // 临时缓存目录 'C:/xx/xx/node_modules/.vite/deps_temp'
  ...
  // 创建 package.json，里面type:module，因为所有的缓存文件都应被识别为ES模块
  fs.writeFileSync(
  ...
  // 生成由 hash 与 deps 组成的 8 位 browserHash（浏览器文件的请求会携带该 browserHash）
...
export function getDepHash(config: ResolvedConfig, ssr: boolean): string {
  // 拿到对应的 lock 文件路径(package-lock.json bun.lockb pnpm-lock.yaml yarn.lock)
